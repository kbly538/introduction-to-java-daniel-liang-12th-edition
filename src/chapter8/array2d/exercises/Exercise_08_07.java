...
public class Exercise_08_07 {
    public static void main(String[] args) {
        ...
        int p1 = 0, p2 = 1;
        double shortestDistance3d = distance3d(points[p1][0], points[p1][1], points[p1][2],
                points[p2][0], points[p2][1], points[p2][2]);

        for (int i = 0; i < points.length; i++) {
            for (int j = i + 1; j < points.length; j++) {
                double distance3d = distance3d(points[i][0], points[i][1], points[i][2],
                        points[j][0], points[j][1], points[j][2]);

                if (shortestDistance3d > distance3d) {
                    p1 = i;
                    p2 = j;
...
/* OUTPUT
The closes points are: (4.0, 1.0, 1.0) and (3.5, 2.0, -1.0)
* */